...
namespace hunting_the_manticore
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Player 1 enter distance between Manticore and City
            int distanceFromCity = GetDistanceFromCity();
            ...
            int GetDistanceFromCity()
            {
                while (true)
                {
                    Console.Write("Player 1, how far away from the city do you want to station the Manticore? (Number Between 0 and 100) ");
                    distanceFromCity = Convert.ToInt32(Console.ReadLine());
                    if (distanceFromCity <= 100 && distanceFromCity >= 0) return distanceFromCity;
